...
public class LifeCycleCounterServlet extends HttpServlet {
	...
	public LifeCycleCounterServlet() {
		System.out.println("0.LifeCycleCounterServlet() 기본생성자호출[최초요청시 단 한번호출]객체주소:" + this);
	}

	@Override
	public void init(ServletConfig config) throws ServletException {
		super.init(config);
		System.out.println("1.init메쏘드 생성자호출 직후에 단 한번 호출[객체초기화,리소스획득]");
	}
	...
	@Override
	public void destroy() {
		System.out.println("3.destroy()메쏘드-->서블릿객체가 메모리에서 해제되기 직전에 호출[리소스반납]");
	}
}
